...
const labels: Record<string, string> = {
  ...
  draft: "Taslak", published: "Yayında", offers_received: "Teklif alıyor",
  driver_selected: "Şoför seçildi", in_transit: "Taşınıyor", completed: "Tamamlandı", cancelled: "İptal",
  open: "Açık", reviewing: "İnceleniyor", resolved: "Çözüldü", dismissed: "Reddedildi",
...
function Toolbar({ tab, query, setQuery, filter, setFilter, search }: { tab: Tab; query: string; setQuery: (value: string) => void; filter: string; setFilter: (value: string) => void; search: () => void }) {
  const options = tab === "drivers" ? ["pending", "verified", "rejected"] : tab === "users" ? ["active", "blocked"] : tab === "complaints" ? ["open", "reviewing", "resolved", "dismissed"] : ["draft", "published", "offers_received", "driver_selected", "in_transit", "completed", "cancelled"];
...
function LoadTable({ items, openDetail }: { items: AnyRecord[]; openDetail: (kind: DetailKind, id: string) => void }) { return <Table headings={["İlan", "Rota", "Müşteri", "Tutar", "Durum", ""]}>{items.map(item => <tr key={item.load.id}><td><b>{item.load.title}</b><small className="cell-sub">{item.load.id.slice(0, 8)}</small></td><td>{item.load.pickup?.district || item.load.pickup?.city || "—"} → {item.load.delivery?.district || item.load.delivery?.city || "—"}</td><td>{item.customer?.name || "—"}</td><td>{formatMoney(item.load.agreedPriceTl || item.load.basePriceTl)}</td><td><Status value={item.load.status} /></td><td><button className="table-action" onClick={() => void openDetail("load", item.load.id)}>Detay</button></td></tr>)}</Table>; }
function ComplaintTable({ items, openDetail }: { items: AnyRecord[]; openDetail: (kind: DetailKind, id: string) => void }) { return <Table headings={["Şikâyet", "Şikâyet eden", "Şikâyet edilen", "Mesaj", "Durum", ""]}>{items.map(item => <tr key={item.complaint.id}><td><b>{label(item.complaint.reason)}</b><small className="cell-sub">{formatDate(item.complaint.createdAt)}</small></td><td>{item.reporter?.name || "—"}</td><td>{item.reportedUser?.name || "—"}</td><td className="message-cell">{item.message?.body || "Silinmiş veya medya mesajı"}</td><td><Status value={item.complaint.status} /></td><td><button className="table-action" onClick={() => void openDetail("complaint", item.complaint.id)}>İncele</button></td></tr>)}</Table>; }
...
function LoadDetail({ data, note, setNote, busy, mutate }: any) { const load = data.load; const [status, setStatus] = useState("cancelled"); return <><InfoGrid items={[["Müşteri", data.customer?.name || "—"], ["Şoför", data.driver?.name || "Atanmadı"], ["Rota", `${load.pickup?.address} → ${load.delivery?.address}`], ["Mesafe", `${load.estimatedKm || 0} km`], ["Tutar", formatMoney(load.agreedPriceTl || load.basePriceTl)], ["Durum", label(load.status)]]} /><section><h3>Durum geçmişi</h3><div className="timeline">{(data.statusHistory || []).map((event: AnyRecord) => <div key={event.id}><i /><span><b>{label(event.toStatus)}</b><small>{formatDate(event.createdAt)} · {event.actorRole}</small>{event.note && <p>{event.note}</p>}</span></div>)}</div></section><section className="action-box"><h3>Yönetici durum güncellemesi</h3><p>Operasyonel akışı yalnızca gerekli durumlarda ve kayıtlı bir gerekçeyle değiştirin.</p><select value={status} onChange={event => setStatus(event.target.value)}>{["draft", "published", "offers_received", "driver_selected", "in_transit", "completed", "cancelled"].filter(value => value !== load.status).map(value => <option key={value} value={value}>{label(value)}</option>)}</select><textarea value={note} onChange={event => setNote(event.target.value)} placeholder="İşlem notu (zorunlu)" /><button className={status === "cancelled" ? "danger-button" : "primary-button"} disabled={busy || !note.trim()} onClick={() => void mutate(`/loads/${load.id}/status`, { status, note }, "İlan durumu güncellendi.")}>Durumu güncelle</button></section></>; }

function ComplaintDetail({ data, note, setNote, busy, mutate }: any) { const complaint = data.complaint; const [removeMessage, setRemoveMessage] = useState(false); return <><InfoGrid items={[["Şikâyet eden", data.reporter?.name || "—"], ["Şikâyet edilen", data.reportedUser?.name || "—"], ["Neden", label(complaint.reason)], ["Tarih", formatDate(complaint.createdAt)], ["İlan", data.load?.title || "—"], ["Durum", label(complaint.status)]]} /><section><h3>Şikâyet açıklaması</h3><blockquote>{complaint.detail || "Açıklama eklenmemiş."}</blockquote></section><section><h3>Şikâyet edilen mesaj</h3><div className="reported-message"><small>{data.message?.senderRole ? label(data.message.senderRole) : "Gönderen"}</small><p>{data.message?.body || "Mesaj silinmiş veya medya içeriği."}</p><time>{formatDate(data.message?.createdAt)}</time></div></section><section className="action-box"><h3>İncelemeyi sonuçlandır</h3><textarea value={note} onChange={event => setNote(event.target.value)} placeholder="Karar ve gerekçe (zorunlu)" /><label className="check"><input type="checkbox" checked={removeMessage} onChange={event => setRemoveMessage(event.target.checked)} /> Mesaj içeriğini kullanıcıların sohbetinden kaldır</label><div className="button-row"><button className="primary-button" disabled={busy || !note.trim()} onClick={() => void mutate(`/complaints/${complaint.id}`, { status: "resolved", resolutionNote: note, removeMessage }, "Şikâyet çözüldü.")}>Çözüldü olarak kapat</button><button className="outline-button" disabled={busy || !note.trim()} onClick={() => void mutate(`/complaints/${complaint.id}`, { status: "dismissed", resolutionNote: note, removeMessage: false }, "Şikâyet reddedildi.")}>Şikâyeti reddet</button></div></section></>; }
