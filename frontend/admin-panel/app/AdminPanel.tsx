...
const labels: Record<string, string> = {
  ...
  draft: "TASLAK", published: "YAYINDA", offers_received: "YAYINDA", open: "YAYINDA",
...
function Toolbar({ tab, query, setQuery, filter, setFilter, search }: { tab: Tab; query: string; setQuery: (value: string) => void; filter: string; setFilter: (value: string) => void; search: () => void }) {
  const options = tab === "drivers" ? ["pending", "verified", "rejected"] : tab === "users" ? ["active", "blocked"] : tab === "complaints" ? ["open", "reviewing", "resolved", "dismissed"] : ["draft", "published", "driver_selected", "driver_en_route", "at_pickup", "picked_up", "en_route_to_delivery", "delivered", "completed", "cancelled"];
